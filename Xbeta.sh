...
echo -e "${lb}
									    +-+-+-+-+ +-+-+-+-+-+-+-+ +-+-+-+-+ +-+-+-+ +-+-+-+-+-+-+-+-+-+ +-+-+-+
									    |B|E|T|A| |V|E|R|S|I|O|N| |D|O|E|S| |N|O|T| |R|E|P|R|E|S|E|N|T| |T|H|E|
									    +-+-+-+-+ +-+-+-+-+-+-+-+ +-+-+-+-+ +-+-+-+ +-+-+-+-+-+-+-+-+-+ +-+-+-+
												  +-+-+-+-+-+ +-+-+-+-+-+-+-+
												  |F|I|N|A|L| |Q|U|A|L|I|T|Y|
												  +-+-+-+-+-+ +-+-+-+-+-+-+-+
...
process_input() {
	...
	nmap -sn $ip_range
        ;;
        21)
	echo "Enter the name of the fake wifi access point:"
	read ssid
	echo "Enter the password for the fake wifi access point:"
	read password
	echo "Which wifi adapter would you like to use?"
	read adapter
	mode=$(iwconfig $adapter | grep -o "Mode:.*" | cut -d ':' -f 2 | cut -d ' ' -f 1)
	...
	airbase-ng -e $ssid -c 6 -W 1 -z 2 -w $password ${adapter}mon || echo "[-] FAILED: Could not create fake wifi access point"
	ifconfig $adapter down
	...
	echo "Enter the name of the handshake file to crack:"
	read handshake
	wifite --iface $adapter -mac -c 1 --crack --dict /path/to/wordlist --handshake $handshake
	ip=$(ifconfig $adapter | grep 'inet ' | awk '{print $2}')
	echo "IP address of $adapter: $ip"
	ifconfig $adapter down
	iwconfig $adapter mode managed
	ifconfig $adapter up
        ;;
